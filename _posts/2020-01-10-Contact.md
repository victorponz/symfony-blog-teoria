...
Ya solo queda renderizarlos. Para ello, ve a la plantilla `page/contact.twig.php` y localiza el formulario. Sustitúyelo por el siguiente código:
...
Ahora mismo el campo `Email` es de tipo `text`. Vamos a modificar el formulario para que renderice un campo de tipo `email`. Modifica a clase `ContactFormType` :
...
En este momento hay campos en la parte del cliente que tienen el atributo `required` por lo que no se puede enviar el formulario si no se han rellenado. Pero esto se puede eliminar con las herramientas del desarrollador del navegador.

Por ejemplo, vamos a eliminar el atributo `required` del campo `First Name`
...
Ahora mismo cuando se envía un Mensaje, se redirecciona al usuario a la ruta `index`. Modifica el controlador `contact` para que redirija a la ruta `thankyou`.
