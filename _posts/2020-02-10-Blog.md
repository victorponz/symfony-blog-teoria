...
```
Y ahora modificamos todos los controladores que redirijan a esta ruta para añadir el parámetro `slug`:
...
> -alert-**IMPORTANTE**
> Mover este controlador antes de `/blog/{page}` porque de lo contrario salta primero este
...
Plantilla `form_comment.html.twig`:
...
```
Y ahora modificamos la plantilla `blog.html.twig` para recorrer los resultados de la paginación `posts.results` y, de paso, creamos una plantilla para el contenido de cada post `partials/post.html.twig`
{% raw %}
```twig
...
* Una nueva ruta, con la particularidad de que va a tener un parámetro en el querystring `http://127.0.0.1:8080/blog/buscar?search=entrada`
...
Cuando se pulsa el botón `buscar`, se genera la siguiente petición: [http://127.0.0.1:8080/blog/buscar?searchTerm=texto](http://127.0.0.1:8080/blog/buscar?searchTerm=texto). Fíjate que en la url aparece un `queryString`:`searchTerm=texto`. Pues bien, para acceder a dicho parámetro desde un controlador usamos ` $searchTerm = $request->query->get('searchTerm', '');`

Y la **plantilla** `blog.html.twig`, discriminamos si la ruta es `blog` o `blog_buscar` mediante
