...
Y ahora modificamos la plantilla `single_post.html.twig` para crear el enlace a la nueva ruta creada. Lo ideal sería hacerlo con `ajax` para que el usuario no se mueva de la página. A continuación os muestro el código necesario en la parte de Symfony.
{% raw %}
```twig
#[Route('/single_post/{slug}/like', name: 'post_like')]
public function like(ManagerRegistry $doctrine, $slug): JsonResponse
{
    $repository = $doctrine->getRepository(Post::class);
    $post = $repository->findOneBy(["slug"=>$slug]);
    if ($post){
    ...
    $data = ['success' => true, 'likes' => $post->getNumLikes()];
    return $this->json($data);

}
```
Vamos a añadir un `id` al número de likes para porder cogerlo con javascript
{% raw %}
```twig
<li><a href="{{ path('post_like', {slug: post.slug}) }}">LIKE</a> <i class="fa fa-heart sr-icons"></i><span id= 'likes'> {{post.numLikes}}</span></li>
```
{% endraw %}
Y esta la parte cliente 
```javascript
<script type='text/javascript'>
    document.addEventListener('DOMContentLoaded', function() {
        const likeLink = document.querySelector('a[href="{{ path('post_like', {slug: post.slug}) }}"]');
        // Hay que coger el id 'likes' para poder cambiar luego el valor al número de likes actual
